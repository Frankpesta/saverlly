...
@ApiTags('Kiosk Users')
@ApiBearerAuth('jwt')
@ApiParam({ name: 'kioskId', description: 'The kiosk these user accounts belong to' })
...
export class KioskUsersController {
  ...
  @ApiResponse({ status: 403, description: 'Cross-tenant, or a kiosk-owner attempting to assign a disallowed role' })
  create(
  ...
  @ApiResponse({ status: 200, description: 'Users under this kiosk (password hashes never included)' })
  findAll(@Param('kioskId') kioskId: string) {
    return this.kioskUsersService.findAllForKiosk(kioskId);
  }

  @Patch(':userId')
  @ApiOperation({ summary: 'Update a kiosk user — role, disabled flag, or managedLocationIds' })
  @ApiResponse({ status: 200, description: 'User updated' })
  @ApiResponse({ status: 403, description: 'Cross-tenant, or a kiosk-owner acting on a non-location-manager account' })
  @ApiResponse({ status: 404, description: 'User does not belong to this kiosk' })
  update(
    @Param('kioskId') kioskId: string,
    ...
    return this.kioskUsersService.update(kioskId, userId, currentUser.role, dto);
  }
}
