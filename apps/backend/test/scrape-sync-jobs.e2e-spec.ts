...
describe('Scrape + affiliate sync jobs (e2e, real BullMQ + Redis)', () => {
  ...
  it('scrapes real coupon codes via Playwright and dedupes on a repeat run', async () => {
    const merchant = await seedMerchant();

    // Seed directly so this test runs exactly the two explicitly enqueued jobs.
    const source = await testPrisma.scrapeSource.create({
      data: {
        url: fixtureUrl,
    ...
    await waitFor(async () => {
      const count = await testPrisma.coupon.count({
        where: { merchantId: merchant.id },
      });
      if (count !== 2) return false;
      const source = await testPrisma.scrapeSource.findUniqueOrThrow({
    ...
    expect(coupons.map((c) => c.code).sort()).toEqual([
      'SUMMER25',
      'WELCOME10',
    ]);
    ...
    const countAfterRerun = await testPrisma.coupon.count({
      where: { merchantId: merchant.id },
    });
    expect(countAfterRerun).toBe(2);
  }, 60_000);

  it('syncs coupons from the mock affiliate adapter only for merchants with hasCouponApi: true', async () => {
    ...
    const merchantWithApi = await seedMerchant({
      affiliateProgramId: programWithApi.id,
    });
    const merchantNoApi = await seedMerchant({
      affiliateProgramId: programNoApi.id,
    });
    ...
    try {
      ...
      await waitFor(async () => {
        const count = await testPrisma.coupon.count({
          where: { merchantId: merchantWithApi.id },
        });
        return count === 2;
      }, 20_000);
    } finally {
    ...
    const withApiCoupons = await testPrisma.coupon.findMany({
      where: { merchantId: merchantWithApi.id },
    });
    expect(withApiCoupons).toHaveLength(2);
    expect(withApiCoupons.every((c) => c.source === 'API')).toBe(true);

    const noApiCoupons = await testPrisma.coupon.findMany({
      where: { merchantId: merchantNoApi.id },
    });
