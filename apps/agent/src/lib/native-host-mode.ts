...
/**
 * Writes the device token (or an error if not yet registered / currently inactive) to stdout
 * as one framed native message, then returns so the process can exit. This is a push, not a
 * request/response — the extension's port.onMessage fires as soon as Chrome connects the
 * port, no request needed. Checking isAgentActive() here (the last periodic status-sync
 * result), not just token presence, is what fulfills "stop serving token to the extension"
 * once a kiosk/device kill-switch flips — see 04-PHASE-4-desktop-agent.md §5.
 */
export function respondWithDeviceToken(): void {
  ...
  if (!token) {
    ...
    payload = { type: 'device-token', token };
  }
