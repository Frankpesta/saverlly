...
const mockMintAttributionSubId = mintAttributionSubId as jest.MockedFunction<
  typeof mintAttributionSubId
>;
const mockAppendAttributionLog = appendAttributionLog as jest.MockedFunction<
  typeof appendAttributionLog
>;
...
describe("runAttribution", () => {
  beforeEach(() => {
    ...
    chromeMock.storage.session.get.mockImplementation(async (key: string) => ({
      [key]: session[key],
    }));
  ...
  it("does not redirect again when the merchant strips tracking parameters before a retry", async () => {
    mockMintAttributionSubId.mockResolvedValue("click-1");
    const merchant: PublicMerchant = {
      ...baseMerchant,
      affiliateUrlParamKey: "aff",
      affiliateUrlParamValue: "saverlly",
      affiliateSubIdParamKey: "subid",
    };
    await runAttribution(1, "https://shop.example.com/cart", merchant);
    expect(
      await runAttribution(1, "https://shop.example.com/cart", merchant),
    ...
    expect(mockMintAttributionSubId).toHaveBeenCalledTimes(1);
  });

  it("replaces a competing value only when explicit attribution runs", async () => {
    const result = await runAttribution(
      1,
      "https://shop.example.com/cart?aff=competitor",
      {
        ...baseMerchant,
        affiliateUrlParamKey: "aff",
        affiliateUrlParamValue: "saverlly",
    ...
    expect(result).toBe("https://shop.example.com/cart?aff=saverlly");
  });
