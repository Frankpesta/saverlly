...
const mockSetDeviceToken = setDeviceToken as jest.MockedFunction<
  typeof setDeviceToken
>;
const mockSetApiBaseUrl = setApiBaseUrl as jest.MockedFunction<
  typeof setApiBaseUrl
>;
...
describe("connectToAgentAndReceiveToken", () => {
  ...
  it("does not resolve activation before both token and backend URL are stored", async () => {
    ...
    const { port, fireMessage, fireDisconnect } = fakePort();
    const finished = jest.fn();
    const connection = connectToAgentAndReceiveToken(
      jest.fn().mockReturnValue(port),
    ).then(finished);
    fireMessage({
      type: "device-token",
      token: "new",
